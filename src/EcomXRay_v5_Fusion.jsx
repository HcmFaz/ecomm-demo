...
/* ══ STORE VIEW ══════════════════════════════════════════════ */
function StoreView({products,onAdd,wishlist,onWish,fx,lang,xrayMode}) {
  const t=useT();
        ...
        <>
          ...
          <div style={{background:C.cream,borderBottom:`1px solid ${C.border}`,padding:"10px 40px",display:"flex",gap:10,alignItems:"center",position:"sticky",top:0,zIndex:10,boxShadow:"0 2px 8px rgba(44,31,14,0.06)"}}>
            ...
            <div style={{marginLeft:"auto",display:"flex",gap:8,alignItems:"center"}}>
              ...
              {xrayMode&&<div style={{background:`${C.cyan}15`,border:`1px solid ${C.cyan}44`,borderRadius:6,padding:"4px 10px",fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:C.cyan,animation:"glowPulse 2s ease infinite"}}>◈ X-RAY ACTIF</div>}
      ...
      <div style={{height:"100vh",display:"flex",flexDirection:"column",background:C.bg,overflow:"hidden"}} dir={dir}>
        ...
        <div style={{background:C.dark,borderBottom:`1px solid rgba(201,169,110,0.2)`,padding:"0 18px",display:"flex",alignItems:"center",gap:10,height:50,flexShrink:0,zIndex:100}}>
          ...
          <div style={{display:"flex",gap:2,background:"rgba(255,255,255,0.06)",borderRadius:8,padding:2,border:"1px solid rgba(255,255,255,0.1)"}}>
            {MODES.map(m=>{
              const isActive = m.id==="xray" ? xrayMode : (storeTab===m.id && !xrayMode);
              return(
              <button key={m.id} onClick={()=>{
                if(m.id==="xray"){setXrayMode(x=>!x);if(storeTab!=="shop")setStoreTab("shop");}
                else{setStoreTab(m.id);setXrayMode(false);}
              }} style={{background:isActive?"rgba(255,255,255,0.12)":"transparent",border:"1px solid transparent",borderRadius:6,padding:"4px 12px",color:m.id==="xray"&&xrayMode?C.cyan:C.cream,fontSize:11,cursor:"pointer",fontWeight:600,transition:"all .2s",fontFamily:"'Outfit',sans-serif",display:"flex",alignItems:"center",gap:4,opacity:isActive?1:0.7}}
        ...
        <div style={{flex:1,overflow:"hidden",display:"flex",position:"relative"}}>
          {storeTab==="shop"&&<StoreView products={products} onAdd={addToCart} wishlist={wishlist} onWish={toggleWishlist} fx={fx} lang={lang} xrayMode={xrayMode}/>}
