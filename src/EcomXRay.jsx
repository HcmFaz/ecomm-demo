...
const CSS = `
@import url('https://fonts.googleapis.com/css2?family=Syne:wght@400;600;700;800&family=JetBrains+Mono:wght@400;500;600&family=Outfit:wght@300;400;500;600&family=Playfair+Display:ital,wght@0,700;1,400;1,700&display=swap');
...
@keyframes taglineIn{from{opacity:0;letter-spacing:0.4em}to{opacity:1;letter-spacing:0.12em}}
@keyframes imgFloat{0%,100%{transform:translateY(0) scale(1)}50%{transform:translateY(-6px) scale(1.02)}}
@keyframes borderGlow{0%,100%{box-shadow:0 0 0 0 rgba(201,169,110,0)}50%{box-shadow:0 0 24px 2px rgba(201,169,110,0.18)}}
.fu{animation:fadeUp .4s cubic-bezier(.22,.68,0,1.2) forwards}
.fi{animation:fadeIn .3s ease forwards}
.ru{animation:revealUp .5s cubic-bezier(.22,.68,0,1.2) forwards}
...
.card-hover{transition:all .35s cubic-bezier(.22,.68,0,1.2);}
.card-hover:hover{transform:translateY(-6px);box-shadow:0 20px 48px rgba(0,0,0,0.18),0 0 0 1px rgba(201,169,110,0.15);}
`;
...
const PRODUCTS=[
  {id:1,name:"Caftan Brodé Royal",cat:"Mode",price:1850,rating:4.8,rCount:124,stock:3,badge:"Bestseller",emoji:"👘",
   tagline:"Quarante heures entre les mains d'un maître.",
   origin:"Fès · Médina · Quartier des Tanneurs",
   colors:["Bordeaux","Bleu nuit","Ivoire"],sizes:["S","M","L","XL"],
   artisanId:1,
   desc:"Un vêtement n'est pas toujours un vêtement. Celui-ci est le résultat de quarante à quatre-vingts heures de travail concentré — fil d'or sur velours, point par point, motif par motif. La doublure de soie effleure la peau. Les broderies géométriques racontent les mathématiques de l'art islamique.",
   process:"Le velours est tendu sur un cadre en bois. Abdellah trace les motifs à la craie, puis brode à l'aiguille fine. Chaque caftan nécessite environ 40 000 points.",
   certifs:["Artisanat Marocain Certifié","Label Qualité Fès","Origine Traçable"],
   media:["📸 8 photos HD","🎬 Vidéo 3min","📐 Guide tailles","🏅 Certificat"],
   reviews:[{n:"Salma B.",s:5,t:"La broderie est d'une précision stupéfiante. Ma fille était la plus belle du mariage.",v:true,d:"12 jan"},{n:"Karim M.",s:5,t:"Livraison en 3 jours, emballage digne d'une maison de couture.",v:true,d:"8 jan"},{n:"Nadia O.",s:4,t:"Très beau, couleur légèrement différente de la photo — en mieux.",v:false,d:"3 jan"}]},
  {id:2,name:"Argan Pur Bio 100ml",cat:"Beauté",price:280,rating:4.9,rCount:312,stock:45,badge:"Top Ventes",emoji:"🫙",
   tagline:"L'or liquide des arganiers millénaires.",
   origin:"Tiznit · Anti-Atlas · Altitude 800m",
   colors:["Standard"],sizes:["50ml","100ml","200ml"],
   artisanId:2,
   desc:"L'arganier ne pousse nulle part ailleurs qu'ici, entre l'Anti-Atlas et l'océan. Vingt-huit femmes de la coopérative Amal cassent les noix à la main, extraient l'amande, pressent à froid. Sans chaleur. Sans solvants. Sans précipitation. Ce qui sort est vivant.",
   process:"De la collecte à la presse, tout est manuel. Une femme produit environ 1 litre d'huile par jour. Ce flacon représente une journée entière.",
   certifs:["ECOCERT Bio","Fair Trade","Coopérative Féminine","Origine AOC Argane"],
   media:["📸 6 photos","🧪 Analyse labo","🏅 Certificat bio"],
   reviews:[{n:"Leila H.",s:5,t:"Résultats visibles en une semaine. Je ne reviendrai jamais en arrière.",v:true,d:"15 jan"},{n:"Sara T.",s:5,t:"4ème commande. Je n'achète plus rien d'autre pour mes cheveux.",v:true,d:"5 jan"},{n:"Rim A.",s:5,t:"Certifié bio, prix juste, impact réel. Parfait.",v:true,d:"28 déc"}]},
  {id:3,name:"Tajine Terre Cuite",cat:"Maison",price:420,rating:4.7,rCount:89,stock:23,badge:"Artisanat",emoji:"🍲",
   tagline:"La terre de Safi transformée en cuisson lente.",
   origin:"Safi · Côte Atlantique · Capitale de la Poterie",
   colors:["Ocre","Blanc"],sizes:["24cm","28cm","32cm"],
   artisanId:3,
   desc:"Safi est à la poterie ce que Bordeaux est au vin — une géographie qui n'existe nulle part ailleurs. La terre rouge de la région, la technique du tour à pied, les pigments naturels : tout conspire à créer un objet qui cuit mieux que n'importe quel ustensile industriel.",
   process:"Extraction de la terre locale, pétrissage à la main, tournage lent, séchage deux semaines, cuisson au four traditionnel à 900°C. Peinture à la main avant le dernier vernissage.",
   certifs:["Indication Géographique Safi","Artisanat Certifié","Sans plomb ni cadmium"],
   media:["📸 5 photos","🎬 Fabrication","📋 Guide entretien"],
   reviews:[{n:"Youssef D.",s:5,t:"Le parfum du repas est incomparable. La cuisson lente révèle tout.",v:true,d:"20 jan"},{n:"Fatima Z.",s:4,t:"Offert en cadeau. La réaction du destinataire : les larmes aux yeux.",v:true,d:"14 jan"}]},
  {id:4,name:"Babouches Cuir Naturel",cat:"Mode",price:350,rating:4.6,rCount:201,stock:31,badge:"",emoji:"🥿",
   tagline:"Fès coud encore à la main. Chaque point le prouve.",
   origin:"Fès · Bab Boujloud · Souk des Cordonniers",
   colors:["Naturel","Noir","Camel","Miel"],sizes:["38","39","40","41","42","43","44"],
   artisanId:4,
   desc:"Le cuir végétal de l'atelier Bab Boujloud ne connaît pas les raccourcis. Tanné à l'ancienne dans les fameuses cuves de la médina de Fès — safran pour le jaune, coquelicot pour le rouge, indigo pour le bleu — il se patine avec les années pour devenir plus beau.",
   process:"Tannage végétal 15 jours. Découpe au ciseau. Couture main au fil de chanvre. La semelle est cousue, jamais collée. Durée de vie estimée : 10–15 ans avec entretien.",
   certifs:["Tannage Naturel Certifié","Label Médina de Fès","Zéro chimie"],
   media:["📸 8 photos","🎨 12 coloris","📏 Guide tailles"],
   reviews:[{n:"Omar L.",s:5,t:"Confort immédiat dès le premier jour. Le cuir s'adapte au pied comme une seconde peau.",v:true,d:"18 jan"},{n:"Rim A.",s:4,t:"La qualité du cuir parle d'elle-même. Investissement justifié.",v:false,d:"9 jan"}]},
  {id:5,name:"Thé à la Menthe Premium",cat:"Épicerie",price:95,rating:4.9,rCount:445,stock:120,badge:"Favori",emoji:"🍵",
   tagline:"Un rituel d'hospitalité dans une boîte.",
   origin:"Beni Mellal · Piémont du Moyen Atlas",
   colors:["Signature"],sizes:["50g","100g","200g"],
   artisanId:5,
   desc:"Au Maroc, le thé n'est pas une boisson. C'est un protocole, une générosité, une manière de dire : tu es le bienvenu ici, le temps n'existe plus. Ce mélange de gunpowder et de menthe fraîche séchée à l'ombre — jamais au soleil — est la formule que les familles gardent secrète.",
   process:"Sélection manuelle des feuilles de gunpowder. Séchage naturel de la menthe pendant 48h à l'ombre. Assemblage et pesage à la main. Conditionnement en boîte cadeau hermétique.",
   certifs:["Agriculture Raisonnée","Sélection Manuelle","Origine Contrôlée"],
   media:["📸 3 photos","🌿 Origine certifiée","🎁 Packaging cadeau"],
   reviews:[{n:"Hassan M.",s:5,t:"Le seul thé que je peux boire sans sucre. L'équilibre est parfait.",v:true,d:"22 jan"},{n:"Zineb R.",s:5,t:"Acheté pour offrir. Résultat : tout le monde veut l'adresse.",v:true,d:"17 jan"}]},
  {id:6,name:"Coussin Berbère Tissé",cat:"Maison",price:680,rating:4.7,rCount:67,stock:14,badge:"Nouveau",emoji:"🛋️",
   tagline:"Les symboles de protection de l'Atlas vivent dans votre salon.",
   origin:"Haut Atlas · Aït Benhaddou · Altitude 1 200m",
   colors:["Naturel","Rouge","Ocre"],sizes:["40×40cm","50×50cm"],
   artisanId:6,
   desc:"Chaque losange, chaque ligne brisée, chaque motif en zigzag est un mot en amazigh. Fatima tisse des phrases de protection, de fertilité, de voyage. Ce coussin n'est pas une décoration. C'est un texte que la laine raconte.",
   process:"Laine des brebis du Haut Atlas, teinte aux plantes locales. Métier à tisser vertical horizontal traditionnel. Chaque coussin prend 3 à 5 jours. Aucun deux ne sont identiques.",
   certifs:["Artisanat Amazigh Certifié","Laine Naturelle 100%","Teintures Végétales"],
   media:["📸 6 photos","🎬 Artisane","🗺️ Haut-Atlas"],
   reviews:[{n:"Dounia S.",s:5,t:"Il est devenu la pièce dont tout le monde parle dans mon salon.",v:true,d:"11 jan"},{n:"Anas K.",s:5,t:"On sent l'authenticité du travail. Impossible à trouver ailleurs.",v:true,d:"4 jan"}]},
  {id:7,name:"Savon Beldi Noir",cat:"Beauté",price:120,rating:4.8,rCount:289,stock:78,badge:"",emoji:"🫧",
   tagline:"Quatre générations de femmes. Une seule formule.",
   origin:"Marrakech · Quartier Bab Doukkala",
   colors:["Standard"],sizes:["200g","400g"],
   artisanId:7,
   desc:"Le hammam marocain n'est pas un lieu de bain. C'est un temple de purification collective. Le savon beldi noir en est le sacrement. La recette des sœurs Moussaoui n'a pas changé depuis leur arrière-grand-mère : huile d'olive vierge, ghassoul de Moulouya, potasse naturelle.",
   process:"Saponification à froid. Maturation 6 semaines en jarres de terre. Texture finale : semi-liquide, noire profonde, richissime en glycérine naturelle. Sans aucun conservateur.",
   certifs:["Recette Ancestrale","Zéro Conservateur","Huile d'Olive Vierge","Ghassoul AOC"],
   media:["📸 4 photos","🧪 Composition","♻️ Emballage éco"],
   reviews:[{n:"Meryem B.",s:5,t:"Ma peau après l'utilisation : une texture que je n'avais jamais ressentie.",v:true,d:"19 jan"},{n:"Ines C.",s:5,t:"Le parfum seul justifie l'achat. Le reste est un bonus.",v:true,d:"13 jan"}]},
  {id:8,name:"Plateau Zellige Doré",cat:"Maison",price:950,rating:4.9,rCount:43,stock:4,badge:"Exclusif",emoji:"✨",
   tagline:"Pièce N°— d'une édition limitée à 50. Signée.",
   origin:"Fès · Quartier des Potiers · Patrimoine UNESCO",
   colors:["Doré","Argenté"],sizes:["30cm","40cm","50cm"],
   artisanId:8,
   desc:"Maâlem Sidi Mohammed n'a pas de machine. Il a ses mains, son marteau, son burin — et cinquante ans de patience. Chaque fragment de zellige est taillé un par un, puis assemblé selon des règles géométriques qui remontent au XIIème siècle. Ce plateau fait partie d'une série numérotée à 50 exemplaires. Il porte son numéro gravé au revers.",
   process:"Fabrication de l'argile cuite émaillée. Taille individuelle de chaque fragment au marteau. Pose sur lit de plâtre. Polissage à la main. Durée totale : 3 semaines par plateau.",
   certifs:["Patrimoine Artisanal UNESCO","Édition Numérotée","Certificat d'Authenticité","Office Artisanat Maroc"],
   media:["📸 10 photos","🔢 N° de série","🎬 Maître zellij"],
   reviews:[{n:"Adil K.",s:5,t:"Un objet de musée dans mon salon. Les visiteurs s'arrêtent toujours devant.",v:true,d:"6 jan"},{n:"Najwa T.",s:5,t:"Offert à des amis français. Ils ont pleuré. Vrai.",v:true,d:"30 déc"}]},
];
...
/* ── ARTISANS ── */
const ARTISANS={
  1:{name:"Abdellah Tazi",city:"Fès · Médina",spec:"Broderie velours",since:"1992",rating:4.9,sales:847,badge:"Maître Artisan",av:"🧑‍🎨",bio:"Maître brodeur de 3ème génération, formé à l'École Nationale des Arts et Métiers de Fès. Chaque caftan nécessite 40 à 80h de travail exclusivement manuel.",quote:"Je ne brode pas des motifs. Je transmets une géométrie sacrée que mon père m'a apprise."},
  2:{name:"Coopérative Amal",city:"Tiznit · Anti-Atlas",spec:"Huile d'argan bio",since:"2008",rating:5.0,sales:3200,badge:"Certifiée ECOCERT",av:"👩‍🌾",bio:"28 femmes amazighes. Production 100% manuelle, première pression à froid. Certifiée ECOCERT depuis 2010.",quote:"Chaque femme produit environ 1 litre par jour. Ce flacon, c'est sa journée entière."},
  3:{name:"Rachid El Khamlichi",city:"Safi · Côte Atlantique",spec:"Poterie & Céramique",since:"1998",rating:4.8,sales:520,badge:"Artisan Certifié",av:"🏺",bio:"Fils de potier, formé dans les souks de Safi depuis l'âge de 12 ans. Pigments naturels authentiques uniquement.",quote:"La terre de Safi a une mémoire. Elle retient la chaleur différemment de toutes les autres."},
  4:{name:"Atelier Bab Boujloud",city:"Fès Medina · Souk",spec:"Maroquinerie",since:"1985",rating:4.7,sales:1890,badge:"Patrimoine Vivant",av:"🥿",bio:"Atelier classé Patrimoine Artisanal Vivant. Cuir végétal tanné à l'ancienne. Zéro produits chimiques.",quote:"Une semelle cousue main dure dix ans. Une semelle collée : deux ans. Le choix est simple."},
  5:{name:"Plantation Beni Mellal",city:"Beni Mellal · Moyen Atlas",spec:"Thé & Plantes",since:"2001",rating:4.9,sales:5600,badge:"Agriculture Raisonnée",av:"🌿",bio:"12 hectares familiaux. Sélection manuelle des feuilles, séchage naturel à l'ombre pendant 48h.",quote:"Le thé que nous ne sélectionnons pas à la main, nous ne le vendons pas."},
  6:{name:"Fatima Ait Benhaddou",city:"Aït Benhaddou · UNESCO",spec:"Tissage berbère",since:"1994",rating:4.8,sales:234,badge:"Artisane Certifiée",av:"🧶",bio:"Tisserande d'Aït Benhaddou. Chaque motif raconte une histoire de la tradition amazighe ancestrale.",quote:"Je ne tisse pas des formes. Je tisse des phrases. Ceux qui savent lire les voient."},
  7:{name:"Sœurs Moussaoui",city:"Marrakech · Bab Doukkala",spec:"Savons naturels",since:"2006",rating:4.9,sales:2100,badge:"Recettes Ancestrales",av:"🫧",bio:"Atelier fondé par deux sœurs. Formules transmises sur 4 générations, sans conservateurs chimiques.",quote:"Notre arrière-grand-mère n'avait pas de laboratoire. Elle avait la patience. C'est la même chose."},
  8:{name:"Maâlem Sidi Mohammed",city:"Fès · Quartier Potiers",spec:"Art du Zellige",since:"1975",rating:5.0,sales:89,badge:"Maître Artisan Royal",av:"✨",bio:"Titulaire du diplôme de l'Office National de l'Artisanat. Chaque plateau est taillé et posé manuellement.",quote:"Je taille 3 000 fragments pour un plateau. Aucun n'est identique à un autre."},
};
...
function StoreView({products,onAdd,wishlist,onWish,fx}) {
  ...
  /* Backgrounds par catégorie — texture artisanale */
  const catBg={
    "Mode":"linear-gradient(145deg,#1C0F08 0%,#2E1A0E 40%,#1C0F08 100%)",
    "Beauté":"linear-gradient(145deg,#0D1408 0%,#1A2410 40%,#0D1408 100%)",
    "Maison":"linear-gradient(145deg,#0E0E18 0%,#1A1A2E 40%,#0E0E18 100%)",
    "Épicerie":"linear-gradient(145deg,#0F1408 0%,#1E240E 40%,#0F1408 100%)",
    "default":"linear-gradient(145deg,#180E06 0%,#2C1A0A 40%,#180E06 100%)",
  };
  const catAccent={"Mode":C.gold,"Beauté":"#86EFAC","Maison":C.violet,"Épicerie":"#6EE7B7","default":C.gold};

  return (
    <div style={{flex:1,overflowY:"auto",background:"#F5F2EC"}}>

      {/* Bandeau promo élégant */}
      {promoVis&&(
        <div style={{background:"#1A1410",padding:"9px 40px",display:"flex",alignItems:"center",justifyContent:"center",gap:16,position:"relative"}}>
          <div style={{width:4,height:4,borderRadius:"50%",background:C.gold}}/>
          <span style={{fontFamily:"'Outfit',sans-serif",fontSize:11,color:"rgba(255,255,255,0.7)",letterSpacing:"0.04em"}}>
            Code <span style={{color:C.gold,fontWeight:700,letterSpacing:"0.08em"}}>ENCG2025</span> — 10% sur votre première commande
            <span style={{color:"rgba(255,255,255,0.35)",margin:"0 10px"}}>·</span>
            Livraison offerte dès 500 MAD
          </span>
          <div style={{width:4,height:4,borderRadius:"50%",background:C.gold}}/>
          <button onClick={()=>setPromoVis(false)} style={{position:"absolute",right:18,background:"transparent",border:"none",color:"rgba(255,255,255,0.3)",cursor:"pointer",fontSize:16,lineHeight:1}}>×</button>
        </div>
      )}

      {/* HERO — Grande phrase éditoriale */}
      <div style={{background:"#1A1410",padding:"48px 40px 40px",position:"relative",overflow:"hidden",borderBottom:"1px solid rgba(201,169,110,0.12)"}}>
        ...
        {/* Cercle radial subtil */}
        <div style={{position:"absolute",top:"50%",right:"5%",transform:"translateY(-50%)",width:400,height:400,borderRadius:"50%",background:`radial-gradient(circle,${C.gold}08,transparent 65%)`,pointerEvents:"none"}}/>

        <div style={{position:"relative",maxWidth:1020,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",gap:32,flexWrap:"wrap"}}>
            <div style={{flex:1,minWidth:300}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
                <div style={{height:1,width:32,background:C.gold,opacity:0.6}}/>
                <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:C.gold,letterSpacing:"0.22em",opacity:0.8}}>ARTISANAT MAROCAIN · FAIT MAIN</span>
              </div>
              <h1 style={{fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:38,color:"#FAF7F0",lineHeight:1.15,marginBottom:16}}>
                Pas des produits.<br/>
                <em style={{color:C.gold,fontStyle:"italic"}}>Des gestes transmis</em><br/>
                <span style={{fontWeight:400,fontSize:32,color:"rgba(255,255,255,0.55)"}}>depuis des générations.</span>
              </h1>
              <p style={{fontFamily:"'Outfit',sans-serif",fontSize:13,color:"rgba(255,255,255,0.45)",lineHeight:1.8,maxWidth:480,marginBottom:24}}>
                Chaque objet que vous trouvez ici porte un nom, une ville, un savoir-faire.
                Nous travaillons directement avec les artisans — sans intermédiaire.
              </p>
              {/* Barre de recherche premium */}
              <div style={{position:"relative",maxWidth:440}}>
                <input placeholder="Chercher un artisan, une matière, une région…" value={search} onChange={e=>setSearch(e.target.value)}
                  style={{width:"100%",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(201,169,110,0.25)",borderRadius:3,padding:"13px 44px 13px 18px",fontSize:12,fontFamily:"'Outfit',sans-serif",color:"#FAF7F0",outline:"none",letterSpacing:"0.02em",transition:"border-color .3s"}}
                  onFocus={e=>e.target.style.borderColor="rgba(201,169,110,0.6)"}
                  onBlur={e=>e.target.style.borderColor="rgba(201,169,110,0.25)"}/>
                <span style={{position:"absolute",right:16,top:"50%",transform:"translateY(-50%)",color:C.gold,fontSize:13,opacity:0.6}}>↗</span>
              </div>
            </div>

            {/* Colonne droite — chiffres sobres */}
            <div style={{display:"flex",flexDirection:"column",gap:20,alignItems:"flex-end",minWidth:160}}>
              {[["340","artisans partenaires"],["47","pays de livraison"],["100%","fait à la main"]].map(([v,l])=>(
                <div key={l} style={{textAlign:"right"}}>
                  <div style={{fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:28,color:C.gold,lineHeight:1}}>{v}</div>
                  <div style={{fontFamily:"'Outfit',sans-serif",fontSize:10,color:"rgba(255,255,255,0.35)",letterSpacing:"0.06em",marginTop:3}}>{l.toUpperCase()}</div>
                </div>
              ))}
            </div>
          </div>

          {/* Badges tech — discrets en bas */}
          <div style={{display:"flex",gap:4,flexWrap:"wrap",marginTop:28,paddingTop:20,borderTop:"1px solid rgba(255,255,255,0.06)"}}>
            {[["🔒","TLS 1.3"],["🔑","OAuth 2.0"],["💳","PCI DSS L1"],["🌍","CDN 42 régions"],["🌐","PWA · Ubiquité · Sl.14"]].map(([ico,label])=>(
              <div key={label} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:2,padding:"3px 9px",display:"flex",alignItems:"center",gap:4}}>
                <span style={{fontSize:8}}>{ico}</span>
                <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:7,color:"rgba(255,255,255,0.35)"}}>{label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Barre filtres — épurée */}
      <div style={{background:"#FAF8F4",borderBottom:"1px solid #E8E3DC",padding:"12px 40px",display:"flex",gap:10,alignItems:"center",position:"sticky",top:0,zIndex:10}}>
        <div style={{display:"flex",gap:4}}>
          {CATS.map(c=>(
            <button key={c} onClick={()=>setCat(c)} style={{background:"transparent",border:"none",borderBottom:`2px solid ${cat===c?"#1A1410":"transparent"}`,padding:"4px 12px 6px",fontSize:11,fontWeight:cat===c?700:400,cursor:"pointer",color:cat===c?"#1A1410":"#9B8A6E",fontFamily:"'Outfit',sans-serif",transition:"all .2s",letterSpacing:"0.02em"}}>{c}</button>
          ))}
        </div>
        <div style={{marginLeft:"auto",display:"flex",gap:8,alignItems:"center"}}>
          <select value={sort} onChange={e=>setSort(e.target.value)} style={{border:"none",borderBottom:"1px solid #D1CAC0",padding:"4px 8px",fontSize:11,fontFamily:"'Outfit',sans-serif",color:"#6B5A42",background:"transparent",outline:"none",cursor:"pointer"}}>
            <option value="popular">Les plus populaires</option>
            <option value="rating">Mieux notés</option>
            <option value="price_asc">Prix croissant</option>
            <option value="price_desc">Prix décroissant</option>
          </select>
          {compare.length>0&&<button onClick={()=>setShowCmp(true)} style={{background:"#1A1410",border:"none",borderRadius:2,padding:"5px 12px",color:"#FAFAF7",fontSize:10,cursor:"pointer",fontFamily:"'Outfit',sans-serif",fontWeight:700,letterSpacing:"0.04em"}}>⚖ Comparer ({compare.length})</button>}
          <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:"#C4BAB0"}}>{filtered.length}</span>
        </div>
      </div>

      {/* GRILLE PRODUITS — Format éditorial */}
      <div style={{padding:"32px 40px 60px",maxWidth:1080,margin:"0 auto"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:2}}>
          {loading?[...Array(8)].map((_,i)=>(
            <div key={i} style={{background:"#fff",aspectRatio:"3/4"}}>
              <div className="skel" style={{height:"60%",width:"100%"}}/>
              <div style={{padding:"16px",display:"flex",flexDirection:"column",gap:8}}>
                <div className="skel" style={{height:8,borderRadius:2,width:"40%"}}/>
                <div className="skel" style={{height:14,borderRadius:2,width:"80%"}}/>
                <div className="skel" style={{height:10,borderRadius:2,width:"60%"}}/>
              </div>
            </div>
          )):filtered.map((p,i)=>{
            ...
            const accent=catAccent[p.cat]||catAccent.default;
            const bg=catBg[p.cat]||catBg.default;
            return (
              <div key={p.id}
                style={{background:"#fff",border:`1px solid ${inC?"#1A1410":"transparent"}`,overflow:"hidden",transition:"all .4s cubic-bezier(.22,.68,0,1.2)",boxShadow:isHovered?"0 24px 60px rgba(0,0,0,0.14),0 2px 0 rgba(201,169,110,0.3)":"0 1px 3px rgba(0,0,0,0.06)",transform:isHovered?"translateY(-6px)":"none",animation:`fadeUp .45s ease ${i*.06}s both`,cursor:"pointer"}}
                onMouseEnter={()=>setHoveredCard(p.id)}
                onMouseLeave={()=>setHoveredCard(null)}
                onClick={()=>setSelected(p)}>

                {/* Zone image — fond sombre texturé */}
                <div style={{height:220,background:bg,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden"}}>
                  {/* Cercle d'ambiance */}
                  <div style={{position:"absolute",inset:0,background:`radial-gradient(circle at 60% 40%,${accent}10,transparent 65%)`,transition:"opacity .4s",opacity:isHovered?1:0.5}}/>
                  {/* Emoji produit — grand format */}
                  <span style={{fontSize:76,position:"relative",zIndex:1,transition:"transform .4s cubic-bezier(.22,.68,0,1.2)",transform:isHovered?"scale(1.08) translateY(-4px)":"scale(1)",filter:"drop-shadow(0 8px 24px rgba(0,0,0,0.4))"}}>{p.emoji}</span>
                  {/* Origine — discret */}
                  <div style={{position:"absolute",bottom:10,left:12,right:12,display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
                    <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:7,color:"rgba(255,255,255,0.4)",letterSpacing:"0.1em"}}>{p.origin?.split("·")[0].trim()}</div>
                    <button onClick={e=>{e.stopPropagation();onWish(p);}} style={{background:"rgba(255,255,255,0.1)",border:`1px solid rgba(255,255,255,${inW?0.4:0.1})`,borderRadius:"50%",width:28,height:28,cursor:"pointer",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",transition:"all .3s",backdropFilter:"blur(4px)"}}>{inW?"❤️":"🤍"}</button>
                  </div>
                  {/* Badge */}
                  {p.badge&&<div style={{position:"absolute",top:10,left:10,background:p.badge==="Exclusif"?C.gold:p.badge==="Bestseller"?"#FAF7F0":"rgba(255,255,255,0.9)",color:p.badge==="Exclusif"?"#1A1410":p.badge==="Bestseller"?"#1A1410":"#059669",fontSize:8,fontWeight:800,padding:"3px 8px",letterSpacing:"0.1em",fontFamily:"'Outfit',sans-serif"}}>{p.badge.toUpperCase()}</div>}
                  {low&&<div style={{position:"absolute",top:10,right:10,background:"rgba(239,68,68,0.9)",color:"#fff",fontSize:7,fontWeight:700,padding:"3px 7px",letterSpacing:"0.08em",fontFamily:"'JetBrains Mono',monospace"}}>+QUE {p.stock}</div>}
                  {/* Barre comparaison */}
                  <button onClick={e=>{e.stopPropagation();toggleCmp(p);}} style={{position:"absolute",top:10,right:low?70:10,background:inC?"rgba(201,169,110,0.9)":"rgba(255,255,255,0.1)",border:"none",borderRadius:2,padding:"3px 6px",cursor:"pointer",fontSize:9,color:inC?"#1A1410":"rgba(255,255,255,0.5)",transition:"all .2s",fontFamily:"'Outfit',sans-serif",fontWeight:700}} title={inC?"Retirer":"Comparer"}>⚖</button>
                </div>
                ...
                <div style={{padding:"16px 18px 18px"}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                    <span style={{fontFamily:"'Outfit',sans-serif",fontSize:8,color:"#B8A99A",letterSpacing:"0.16em",fontWeight:500}}>{p.cat.toUpperCase()}</span>
                    {artisan&&<span style={{fontFamily:"'Outfit',sans-serif",fontSize:8,color:`${accent}99`,fontStyle:"italic"}}>{artisan.av} {artisan.name.split(" ")[0]}</span>}
                  </div>
                  {/* Nom produit */}
                  <div style={{fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:16,color:"#1A1410",marginBottom:6,lineHeight:1.2}}>{p.name}</div>
                  {/* Tagline audacieuse */}
                  {p.tagline&&<div style={{fontFamily:"'Playfair Display',serif",fontStyle:"italic",fontSize:11,color:"#7A6A5A",lineHeight:1.5,marginBottom:10,fontWeight:400}}>{p.tagline}</div>}
                  {/* Note */}
                  <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:12}}>
                    <span style={{color:C.gold,fontSize:10,letterSpacing:"0.05em"}}>{"★".repeat(Math.floor(p.rating))}</span>
                    <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:8,color:"#C4BAB0"}}>{p.rating} · {p.rCount} avis</span>
                  </div>
                  {/* Prix + CTA */}
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    <span style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:18,color:"#1A1410",letterSpacing:"-0.01em"}}>{fx(p.price)}</span>
                    <div style={{background:isHovered?"#1A1410":"transparent",border:"1px solid #1A1410",borderRadius:2,padding:"7px 14px",fontSize:10,fontWeight:700,cursor:"pointer",color:isHovered?"#FAF7F0":"#1A1410",fontFamily:"'Outfit',sans-serif",transition:"all .35s",letterSpacing:"0.06em"}}>DÉCOUVRIR</div>
                  </div>
                </div>
              </div>
            );
        ...
        {/* Section communauté — format magazine */}
        {!loading&&(
          <div style={{marginTop:48,borderTop:"1px solid #E8E3DC",paddingTop:40}}>
            <div style={{display:"flex",alignItems:"baseline",gap:16,marginBottom:24}}>
              <h2 style={{fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:22,color:"#1A1410"}}>Ce qu'ils en disent</h2>
              <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:8,color:C.violet,letterSpacing:"0.1em"}}>TECH. SOCIALE · SLIDE 21</span>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:16}}>
              {PRODUCTS.flatMap(p=>p.reviews.map(r=>({...r,product:p.name,emoji:p.emoji}))).filter(r=>r.v).slice(0,4).map((r,i)=>(
                <div key={i} style={{background:"#fff",borderLeft:"3px solid #E8E3DC",padding:"18px 20px",animation:`fadeUp .4s ease ${i*.08}s both`}}>
                  <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,color:"#2D1F14",lineHeight:1.7,marginBottom:14,fontStyle:"italic"}}>« {r.t} »</div>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <div style={{width:28,height:28,borderRadius:"50%",background:`linear-gradient(135deg,${C.gold},#8B6914)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:"#fff"}}>{r.n[0]}</div>
                    <div>
                      <div style={{fontFamily:"'Outfit',sans-serif",fontSize:11,fontWeight:600,color:"#1A1410"}}>{r.n}</div>
                      <div style={{display:"flex",alignItems:"center",gap:5}}>
                        <span style={{color:C.gold,fontSize:9}}>{"★".repeat(r.s)}</span>
                        <span style={{fontFamily:"'Outfit',sans-serif",fontSize:9,color:"#9B8A6E"}}>{r.emoji} {r.product}</span>
                      </div>
                    </div>
                    <div style={{marginLeft:"auto",background:"#DCFCE7",borderRadius:2,padding:"2px 6px",fontSize:7,color:"#059669",fontWeight:700,letterSpacing:"0.06em"}}>VÉRIFIÉ</div>
                  </div>
                </div>
              ))}
            </div>
            <div style={{display:"flex",gap:8,marginTop:18,alignItems:"center"}}>
              <span style={{fontFamily:"'Outfit',sans-serif",fontSize:11,color:"#9B8A6E"}}>Partager :</span>
              {[["📱","WhatsApp","#25D366"],["📸","Instagram","#E1306C"],["🐦","X","#1DA1F2"]].map(([ico,name,color])=>(
                <div key={name} style={{border:`1px solid ${color}33`,borderRadius:2,padding:"5px 12px",fontSize:10,color,fontFamily:"'Outfit',sans-serif",fontWeight:600,cursor:"pointer",letterSpacing:"0.04em"}}>{ico} {name}</div>
              ))}
            </div>
          </div>
        )}
...
/* ══ PRODUCT MODAL — Editorial Premium ══════════════════════ */
function ProductModal({product:p,onClose,onAdd,onWish,wishlist,fx}) {
  ...
  const catBg={
    "Mode":"linear-gradient(145deg,#1C0F08,#2E1A0E)",
    "Beauté":"linear-gradient(145deg,#0D1408,#1A2410)",
    "Maison":"linear-gradient(145deg,#0E0E18,#1A1A2E)",
    "Épicerie":"linear-gradient(145deg,#0F1408,#1E240E)",
  };

  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(10,6,3,0.75)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(8px)"}}>
      <div onClick={e=>e.stopPropagation()} className="fu" style={{background:"#FAF8F4",width:620,maxWidth:"95vw",maxHeight:"92vh",overflow:"hidden",display:"flex",flexDirection:"column",boxShadow:"0 40px 100px rgba(0,0,0,0.5)"}}>

        {/* Zone visuelle produit */}
        <div style={{height:240,background:catBg[p.cat]||"linear-gradient(145deg,#1A1410,#2C1A0A)",display:"flex",alignItems:"center",justifyContent:"center",position:"relative",flexShrink:0,overflow:"hidden"}}>
          <div style={{position:"absolute",inset:0,background:`radial-gradient(ellipse at 50% 80%,${C.gold}14,transparent 60%)`}}/>
          <span style={{fontSize:90,position:"relative",zIndex:1,filter:"drop-shadow(0 12px 32px rgba(0,0,0,0.5))",animation:"imgFloat 4s ease-in-out infinite"}}>{p.emoji}</span>
          {/* Boutons flottants */}
          <button onClick={onClose} style={{position:"absolute",top:14,right:14,background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.15)",width:32,height:32,cursor:"pointer",color:"rgba(255,255,255,0.7)",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)",transition:"all .2s"}} onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,0.2)"} onMouseLeave={e=>e.currentTarget.style.background="rgba(255,255,255,0.1)"}>×</button>
          <button onClick={()=>onWish(p)} style={{position:"absolute",top:14,right:54,background:"rgba(255,255,255,0.1)",border:`1px solid rgba(255,255,255,${inW?0.5:0.15})`,width:32,height:32,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)",animation:inW?"heartPop .4s ease":"none"}}>{inW?"❤️":"🤍"}</button>
          {/* Origin pill */}
          {p.origin&&<div style={{position:"absolute",bottom:14,left:16,background:"rgba(0,0,0,0.45)",backdropFilter:"blur(6px)",border:"1px solid rgba(255,255,255,0.1)",padding:"4px 10px",display:"flex",alignItems:"center",gap:5}}>
            <span style={{fontSize:9}}>📍</span>
            <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:8,color:"rgba(255,255,255,0.6)",letterSpacing:"0.1em"}}>{p.origin}</span>
          </div>}
          {p.stock<=5&&<div style={{position:"absolute",bottom:14,right:16,background:"rgba(239,68,68,0.85)",padding:"4px 10px",fontFamily:"'Outfit',sans-serif",fontSize:8,fontWeight:700,color:"#fff",letterSpacing:"0.08em"}}>PLUS QUE {p.stock} EN STOCK</div>}
        </div>

        {/* En-tête produit */}
        <div style={{padding:"20px 28px 0",borderBottom:"1px solid #EDE9E4",flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
            <div style={{flex:1}}>
              <span style={{fontFamily:"'Outfit',sans-serif",fontSize:8,color:"#B8A99A",letterSpacing:"0.16em"}}>{p.cat.toUpperCase()}</span>
              <h2 style={{fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:22,color:"#1A1410",lineHeight:1.2,marginTop:2}}>{p.name}</h2>
              {p.tagline&&<p style={{fontFamily:"'Playfair Display',serif",fontStyle:"italic",fontSize:12,color:"#7A6A5A",marginTop:4,lineHeight:1.5}}>{p.tagline}</p>}
            </div>
            <div style={{textAlign:"right",flexShrink:0,paddingLeft:20}}>
              <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:26,color:"#1A1410",letterSpacing:"-0.02em"}}>{fx(p.price*qty)}</div>
              <div style={{display:"flex",alignItems:"center",gap:4,justifyContent:"flex-end",marginTop:3}}>
                ...
                <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:8,color:"#C4BAB0"}}>{p.rating} · {p.rCount}</span>
              </div>
            </div>
          </div>
          {/* Onglets */}
          <div style={{display:"flex",gap:0,marginTop:12}}>
            {[["story","📖 Histoire"],["artisan","🧑‍🎨 Artisan"],["reviews",`⭐ Avis (${p.reviews.length})`]].map(([id,label])=>(
              <button key={id} onClick={()=>setTab(id)} style={{padding:"9px 16px",background:"transparent",border:"none",borderBottom:`2px solid ${tab===id?"#1A1410":"transparent"}`,cursor:"pointer",fontFamily:"'Outfit',sans-serif",fontSize:11,fontWeight:tab===id?700:400,color:tab===id?"#1A1410":"#9B8A6E",transition:"all .2s",letterSpacing:"0.02em"}}>{label}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"22px 28px"}}>
          ...
          {tab==="story"&&(
            <div>
              {/* Description audacieuse */}
              <p style={{fontFamily:"'Outfit',sans-serif",fontSize:13,color:"#2D1F14",lineHeight:1.9,marginBottom:20}}>{p.desc}</p>

              {/* Processus de fabrication */}
              {p.process&&(
                <div style={{borderLeft:"2px solid #E8E3DC",paddingLeft:18,marginBottom:20}}>
                  <div style={{fontFamily:"'Outfit',sans-serif",fontSize:8,color:"#B8A99A",letterSpacing:"0.14em",marginBottom:8}}>LE PROCESSUS</div>
                  <p style={{fontFamily:"'Outfit',sans-serif",fontSize:12,color:"#4B5563",lineHeight:1.8}}>{p.process}</p>
                </div>
              )}
              ...
              {p.certifs&&(
                <div style={{marginBottom:22}}>
                  <div style={{fontFamily:"'Outfit',sans-serif",fontSize:8,color:"#B8A99A",letterSpacing:"0.14em",marginBottom:10}}>CERTIFICATIONS & GARANTIES</div>
                  <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                    {p.certifs.map(c=>(
                      <div key={c} style={{background:"#F0ECE6",border:"1px solid #DDD8D2",padding:"5px 11px",display:"flex",alignItems:"center",gap:5}}>
                        <span style={{color:"#059669",fontSize:9}}>✓</span>
                        <span style={{fontFamily:"'Outfit',sans-serif",fontSize:10,color:"#4B5563",fontWeight:500}}>{c}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* Options */}
              {p.colors.length>1&&<div style={{marginBottom:16}}>
                <div style={{fontFamily:"'Outfit',sans-serif",fontSize:8,color:"#B8A99A",letterSpacing:"0.14em",marginBottom:8}}>COULEUR — <span style={{color:"#1A1410",fontWeight:600,letterSpacing:"0"}}>{color}</span></div>
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{p.colors.map(c=><button key={c} onClick={()=>setColor(c)} style={{background:color===c?"#1A1410":"transparent",border:`1px solid ${color===c?"#1A1410":"#D1CAC0"}`,padding:"6px 14px",fontSize:11,cursor:"pointer",color:color===c?"#FAF7F0":"#4B5563",transition:"all .2s",fontFamily:"'Outfit',sans-serif"}}>{c}</button>)}</div>
              </div>}
              {size&&<div style={{marginBottom:20}}>
                <div style={{fontFamily:"'Outfit',sans-serif",fontSize:8,color:"#B8A99A",letterSpacing:"0.14em",marginBottom:8}}>TAILLE — <span style={{color:"#1A1410",fontWeight:600,letterSpacing:"0"}}>{size}</span></div>
                <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{p.sizes.map(s=><button key={s} onClick={()=>setSize(s)} style={{background:size===s?"#1A1410":"transparent",border:`1px solid ${size===s?"#1A1410":"#D1CAC0"}`,padding:"5px 12px",fontSize:11,cursor:"pointer",color:size===s?"#FAF7F0":"#4B5563",transition:"all .2s",fontFamily:"'Outfit',sans-serif"}}>{s}</button>)}</div>
              </div>}

              {/* CTA */}
              <div style={{display:"flex",gap:10,alignItems:"center",paddingTop:16,borderTop:"1px solid #EDE9E4"}}>
                <div style={{display:"flex",alignItems:"center",border:"1px solid #D1CAC0",gap:0}}>
                  <button onClick={()=>setQty(Math.max(1,qty-1))} style={{background:"transparent",border:"none",cursor:"pointer",color:"#4B5563",width:32,height:40,fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
                  <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:13,fontWeight:700,color:"#1A1410",padding:"0 12px",borderLeft:"1px solid #D1CAC0",borderRight:"1px solid #D1CAC0",height:40,display:"flex",alignItems:"center"}}>{qty}</span>
                  <button onClick={()=>setQty(Math.min(p.stock,qty+1))} style={{background:"transparent",border:"none",cursor:"pointer",color:"#4B5563",width:32,height:40,fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
                </div>
                <button onClick={handleAdd} style={{flex:1,background:adding?"#2D4A1E":"#1A1410",border:"none",padding:"12px 20px",color:"#FAF7F0",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"'Outfit',sans-serif",transition:"all .3s",letterSpacing:"0.08em",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
          ...
          {tab==="artisan"&&artisan&&(
            <div>
              {/* Header artisan */}
              <div style={{display:"flex",gap:16,alignItems:"flex-start",marginBottom:24,padding:"18px",background:"#F0ECE6"}}>
                <div style={{width:56,height:56,borderRadius:"50%",background:`linear-gradient(135deg,${C.gold},#8B6914)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>{artisan.av}</div>
                <div style={{flex:1}}>
                  <div style={{fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:18,color:"#1A1410",marginBottom:2}}>{artisan.name}</div>
                  <div style={{fontFamily:"'Outfit',sans-serif",fontSize:11,color:"#7A6A5A",marginBottom:6}}>📍 {artisan.city} · Artisan depuis {artisan.since}</div>
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    <div style={{background:"#1A1410",padding:"3px 10px",fontFamily:"'Outfit',sans-serif",fontSize:8,fontWeight:700,color:C.gold,letterSpacing:"0.08em"}}>{artisan.badge.toUpperCase()}</div>
                    <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:"#9B8A6E"}}>{artisan.sales.toLocaleString()} ventes · {artisan.rating}★</div>
                  </div>
                </div>
              </div>
              ...
              {/* Citation */}
              {artisan.quote&&(
                <div style={{borderLeft:"3px solid #C9A96E",paddingLeft:18,marginBottom:22}}>
                  <p style={{fontFamily:"'Playfair Display',serif",fontStyle:"italic",fontSize:15,color:"#2D1F14",lineHeight:1.7}}>« {artisan.quote} »</p>
                </div>
              )}

              <p style={{fontFamily:"'Outfit',sans-serif",fontSize:12,color:"#4B5563",lineHeight:1.9,marginBottom:20}}>{artisan.bio}</p>

              {/* Spécialité */}
              <div style={{background:"#FAF0E0",border:"1px solid #F0DDB8",padding:"14px 16px"}}>
                <div style={{fontFamily:"'Outfit',sans-serif",fontSize:8,color:"#9B7A3A",letterSpacing:"0.14em",marginBottom:6}}>SPÉCIALITÉ</div>
                <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:14,color:"#2D1A06"}}>{artisan.spec}</div>
              </div>

              <div style={{marginTop:16,padding:"12px 16px",background:"#F5F2EC",border:"1px solid #EDE9E4"}}>
                <div style={{fontFamily:"'Outfit',sans-serif",fontSize:10,color:"#7A6A5A",lineHeight:1.7}}>
                  En achetant ce produit, vous rémunérez directement {artisan.name.split(" ")[0]} — sans commission intermédiaire excessive. NouaSell prélève 8–12% uniquement.
                </div>
              </div>
            </div>
          )}
          ...
          {tab==="reviews"&&(
            <div>
              <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:20,padding:"16px",background:"#F5F2EC"}}>
                <div style={{fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:42,color:"#1A1410",lineHeight:1}}>{p.rating}</div>
                <div>
                  <div style={{color:C.gold,fontSize:16,letterSpacing:"0.1em"}}>{"★".repeat(Math.floor(p.rating))}</div>
                  <div style={{fontFamily:"'Outfit',sans-serif",fontSize:11,color:"#7A6A5A",marginTop:3}}>{p.rCount} avis vérifiés</div>
                </div>
                <div style={{marginLeft:"auto",fontFamily:"'JetBrains Mono',monospace",fontSize:7,color:C.violet,letterSpacing:"0.1em",textAlign:"right"}}>👥 TECH. SOCIALE<br/>SLIDE 21</div>
              </div>
              {p.reviews.map((r,i)=>(
                <div key={i} style={{padding:"16px 0",borderBottom:"1px solid #EDE9E4"}}>
                  <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:8}}>
                    <div style={{width:34,height:34,borderRadius:"50%",background:`linear-gradient(135deg,${C.gold},#8B6914)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,color:"#fff",flexShrink:0}}>{r.n[0]}</div>
                    <div style={{flex:1}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
                        <span style={{fontFamily:"'Outfit',sans-serif",fontSize:12,fontWeight:600,color:"#1A1410"}}>{r.n}</span>
                        <span style={{color:C.gold,fontSize:10}}>{"★".repeat(r.s)}</span>
                        {r.v&&<span style={{background:"#DCFCE7",padding:"1px 6px",fontSize:7,color:"#059669",fontWeight:700,letterSpacing:"0.06em"}}>VÉRIFIÉ</span>}
                        <span style={{fontFamily:"'Outfit',sans-serif",fontSize:9,color:"#C4BAB0",marginLeft:"auto"}}>{r.d}</span>
                      </div>
                      <p style={{fontFamily:"'Playfair Display',serif",fontStyle:"italic",fontSize:12,color:"#4B5563",lineHeight:1.7}}>« {r.t} »</p>
                    </div>
                  </div>
                </div>
              ))}
